...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="mb-4 d-flex align-items-center justify-content-between">
        <h4 class="mb-0"><span class="text-muted fw-light">Dashboard / </span><a href="{{url('admin/customer')}}"> Customer </a> / Customer Plan</h4>
        <div>
                            ...
                            <div class="card-header d-flex justify-content-between">
                                ...
                                <div class="col-lg-6">
                                    <div class="alert alert-success">
                                        <strong>Paid:</strong> One-Time Consultation charges of ₹ 699 have been paid.
                                    </div>
                                </div>
                                @elseif($consultation_charges == 2 || $consultation_charges == 0)
                                <div class="col-lg-8">
                                    <div class="alert alert-danger">
                                        <strong>Not Paid:</strong> One-Time Consultation charges are still pending.
                                    </div>
                                </div>
                                @endif
                                @endif
                            </div>
                            <form id="submit_ajax_form" method="POST" route="save-customer-plan">
                                @csrf
                                <div class="ajax-msg"></div>
                                <input type="hidden" name="total_outstanding_amount" value="{{$total_outstanding_amount}}">
                                ...
                                <div class="card-body" id="customerDetailsForm" data-id="step-1">
                                    ...
                                    @if(in_array(4,explode(',', $admin_id['role'])) || in_array(1,explode(',', $admin_id['role'])))

                                    @if(!empty($total_outstanding_amount) && !empty($current_emi) && !empty($total_loan_amt))
                                    <div class="mt-3">
                                        <button class="btn btn-primary" id="" onclick="next_process(); return false;" type="button">Next</button>
                                    </div>
                                ...
                                <div class="card-body d-none" id="paymentOptions" data-id="step-2">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3 mt-3">
                                            <button class="btn btn-primary" id="pay_later" onclick="proceedToPayment(2); return false;" type="button" data-pay-later="2">Pay Later</button>
                                        </div>
                                        <div class="col-lg-4 mt-3">
                                            <button class="btn btn-primary" id="pay_now" onclick="proceedToPayment(1); return false;" type="button" data-pay-now="1">Send payment link</button>
                                        </div>
                                ...
                                <div class="card-body d-none" id="offerDetails" data-id="step-3">
                                    <div class="mt-1">
                                        ...
                                        <div class="col-lg-6">
                                            ...
                                            <span id="customer_plan_offer_err" class="error customer_plan_offer_err small" style="color:red;"></span>
                                        </div>
                                    </div>
                                    <div class="" id="offerDetailsView"></div>
                                    <div class="mt-3">
                                        <button class="btn btn-success submit-button" type="submit">Activate plan now</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered w-100">
                                        ...
                                        <tbody class="text-center">
                                            ...
                                            <tr>
                                                <td>{{++$count}}</td>
                                                <td>{{ $row->customer_settlement_offers->offer_name}}</td>
                                                <td> {{ $row->settlement_amount == 0 ? "--" : "₹ " .number_format($row->settlement_amount)}}</td>
                                                <td> {{ $row->settlement_percentage == 0 ? "--"  : $row->settlement_percentage ." %"}} </td>
                                                <td>₹ {{ $row->customer_plan_offer_id == 1 ? number_format($row->settlement_service_charge) : number_format($row->service_charge)  }}</td>

                                                <td>{{$row->tenure ? $row->tenure." Month ":" -- "  }} </td>
                                                <td> {{$row->emi_amt == 0 ? "--" : " ₹ " .number_format($row->emi_amt) }}</td>
                                                <td> {{$row->total_emi == 0 ? "--" : "₹ ".number_format($row->total_emi) }}</td>
                                                <td>
...
<script>
    ...
    // 
    $("#submit_ajax_form").on('submit', function(e) {
        e.preventDefault();
        clearAjaxErrors();
        ...
        $.ajax({
            ...
            success: function(data) {
                if ($.isEmptyObject(data.error)) {
                    ...
                    if (data.status === 0) {
                        $("#" + form_id).find('.ajax-msg').html('<div class="alert alert-danger d-flex align-items-center" role="alert"><span class="alert-icon text-success me-2"><i class="ti ti-ban ti-xs"></i></span>' + data.msg + '</div>')
                        $(".ajax-msg").fadeOut(2000, function() {});
                    } else {
                        $("#" + form_id).find('.ajax-msg').html('<div class="alert alert-success d-flex align-items-center" role="alert"><span class="alert-icon text-success me-2"><i class="ti ti-check ti-xs"></i></span>' + data.msg + '</div>')
                        $(".ajax-msg").fadeOut(2000, function() {
                            setTimeout(function() {
