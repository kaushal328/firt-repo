...
<div class="content-wrapper">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        <nav aria-label="breadcrumb" class="mb-2 d-flex align-items-center justify-content-between">
            ...
            <a href="{{url('admin/master/bank-creditor/form')}}" class="btn btn-primary">Add bank creditor</a>
        </nav>
                ...
                <div class="card-header border-bottom">
                    <form>
                        <div class="row">
                            ...
                            <div class="col-lg-4">
                                <label for="title" class="form-label">Bank creditor name</label>
                                <input type="text" class="form-control" name="creditors_name" placeholder="Bank creditor name" value="<?= isset($creditors_name) ? $creditors_name : '' ?>" />
                            </div>
                            <div class="col-lg-3">
                                <button type="submit" class="btn btn-primary mt-4">Search</button>
                                <a class="btn btn-warning mt-4" href="{{url('admin/master/bank-creditor')}}">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    <table class="table">
                        ...
                        <tbody class="table-border-bottom-0">
                            ...
                            <tr data-id="{{ base64_encode($val['id']) }}">
                                <td>{{ $index + 1 }}</td>
                                <td><b>{{ $val['creditors_name']  ?? "--"}}</b>
                                </td>
                                <td><b>{{ $val['phone_no']  ?? "--"}}</b>
                                
                                <td>

                                    {{$val['bank_type']['bank_type_name'] ?? "--"}}
                                ...
                                <td>
                                    <div class="d-flex align-items-sm-center justify-content-sm-center">
                                        <a class="btn btn-sm btn-icon" href="{{url('admin/master/bank-creditor/form/'.base64_encode($val['id']))}}"><i class="ti ti-edit text-primary"></i></a>
    ...
    <script>
        ...
        $(document).on('click', '.delete-record', function(e) {
            ...
            if (confirm("Are you sure you want to delete this record?")) {
                const _this = $(this)
                const url = "{{ url('admin/master/bank-creditor/delete') }}";
                $.post(url, {
